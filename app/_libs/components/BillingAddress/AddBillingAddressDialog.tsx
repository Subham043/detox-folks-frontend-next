...
export default function AddBillingAddressDialog(){
    ...
    return <Dialog open={isOpen} onOpenChange={(open)=>setIsOpen(open)}>
        <DialogTrigger asChild>
            <button className=" text-sm md:text-base py-1 px-3 bg-[#8c6d52] text-white text-center font-semibold rounded-sm transition-all hover:bg-gray-600" onClick={()=>setIsOpen(true)}>ADD ADDRESS</button>
        </DialogTrigger>
